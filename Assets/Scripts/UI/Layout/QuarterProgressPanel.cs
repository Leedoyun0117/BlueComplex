using System;
using TMPro;
using UnityEngine;
using UnityEngine.EventSystems;
using UnityEngine.UI;

namespace BlueComplex.UI.Layout
{
    /// <summary>
    /// 쿼터 진행 스테이터스 창(UI 가이드 12번) — 목업의 노란 포스트잇. "대화 / 쿼터 N / 전체" 글자와, 현재 쿼터의 턴 수만큼 칸이 있고
    /// 현재 턴의 칸이 어둡게 칠해지며 하얀 점이 놓인다. 마지막 칸은 노랑(키를 얻는 턴).
    /// 마우스를 올리면 테두리가 켜지고, 클릭하면 <see cref="Clicked"/>가 온다 —
    /// 전체 스테이지 오버레이를 여는 건 이 패널이 아니라 듣는 쪽이다. 표시만 한다 — 판정 없음.
    /// </summary>
    public sealed class QuarterProgressPanel : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IPointerClickHandler
    {
        /// <summary>포스트잇은 노란 종이라 호버 테두리는 흰색이 아니라 어두운 갈색으로 켠다(흰 테두리는 종이 위에서 안 보인다).</summary>
        private static readonly Color IdleBorder = new Color32(70, 56, 20, 0);
        private static readonly Color HoverBorder = new Color32(70, 56, 20, 255);

        private const float BorderThickness = 2f;
        private const float TiltDegrees = 2f;

        private Image _border;
        private TurnTrack _track;
        private TMP_Text _counter;

        public event Action Clicked;

        public static QuarterProgressPanel Create(Transform parent, int turnsPerQuarter, Vector2 anchorMin, Vector2 anchorMax,
            TMP_FontAsset font)
        {
            var rect = RuntimeUi.CreateRect(parent, "Quarter Progress Panel", anchorMin, anchorMax, Vector2.zero, Vector2.zero);
            rect.localRotation = Quaternion.Euler(0f, 0f, TiltDegrees);
            var panel = rect.gameObject.AddComponent<QuarterProgressPanel>();
            panel.Build(turnsPerQuarter, font);
            return panel;
        }

        /// <summary>현재 쿼터 안에서 지금이 몇 번째 턴인지(1부터, 0이면 점 없음).</summary>
        public void SetTurn(int turnInQuarter, bool animate) => _track.SetTurn(turnInQuarter, animate);

        /// <summary>"2 / 3" 글자. 쿼터가 시작되기 전(0)에는 1로 보여 준다.</summary>
        public void SetQuarter(int quarter, int quarterCount) =>
            _counter.text = $"{Mathf.Max(quarter, 1)} / {quarterCount}";

        public void OnPointerEnter(PointerEventData eventData) => _border.color = HoverBorder;

        public void OnPointerExit(PointerEventData eventData) => _border.color = IdleBorder;

        public void OnPointerClick(PointerEventData eventData)
        {
            if (eventData.button == PointerEventData.InputButton.Left) Clicked?.Invoke();
        }

        private void Build(int turnsPerQuarter, TMP_FontAsset font)
        {
            // 테두리는 루트 이미지가 맡는다: 평소엔 투명, 호버 시 어두운 갈색. 안쪽 몸체(포스트잇)를 테두리 두께만큼 들여 그 위에 얹는다.
            // 투명해도 레이캐스트는 받는다(알파 히트 테스트를 안 쓴다).
            _border = gameObject.AddComponent<Image>();
            _border.color = IdleBorder;
            _border.raycastTarget = true;

            var body = RuntimeUi.CreateRect(transform, "Body", Vector2.zero, Vector2.one,
                new Vector2(BorderThickness, BorderThickness), new Vector2(-BorderThickness, -BorderThickness));
            var bodyImage = body.gameObject.AddComponent<Image>();
            bodyImage.color = MockupStyle.Sticky;
            bodyImage.raycastTarget = false;
            MockupStyle.AddShadow(body.gameObject);

            RuntimeUi.CreateText(body, "Title", "대화", font, 26f, MockupStyle.Ink, TextAlignmentOptions.Center,
                new Vector2(0f, 0.62f), new Vector2(1f, 0.94f)).fontStyle = FontStyles.Bold;
            _counter = RuntimeUi.CreateText(body, "Counter", "1 / 1", font, 26f, MockupStyle.Ink, TextAlignmentOptions.Center,
                new Vector2(0f, 0.34f), new Vector2(1f, 0.64f));
            _counter.fontStyle = FontStyles.Bold;

            _track = new TurnTrack(body, "Track", new Vector2(0.08f, 0.08f), new Vector2(0.92f, 0.30f), turnsPerQuarter, 12f,
                TurnTrackStyle.Sticky);
        }

        private void OnDisable()
        {
            if (_border != null) _border.color = IdleBorder;
            _track?.Kill();
        }
    }
}
