...
namespace BlueComplex.Core.Clues
{
    ...
    {
        /// <summary>목표 구역 쪽 감정을 충분히 가진 단서를 손패에 최소 몇 장 넣을지.</summary>
        public int MinClues { get; }
        ...
        public KeyHandBiasSettings(int minClues, int minTagsNormal, int minTagsVery)
        {
    ...
    public sealed class KeyZoneHandBiasRule
    {
        ...
        public HandBias For(KeyZone target)
        {
            ...
            switch (state)
            {
                case HeartbeatState.VeryDepressed: side = Polarity.Depressed; minTags = _settings.MinTagsVery; break;
                case HeartbeatState.Depressed: side = Polarity.Depressed; minTags = _settings.MinTagsNormal; break;
                case HeartbeatState.Excited: side = Polarity.Excited; minTags = _settings.MinTagsNormal; break;
                case HeartbeatState.VeryExcited: side = Polarity.Excited; minTags = _settings.MinTagsVery; break;
                default: return null;
            }

            return new HandBias(_settings.MinClues, definition => TagsOnSide(definition, side) >= minTags);
        }
